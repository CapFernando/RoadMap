   ...
   e ninguem saberia dizer por que. */
{
  ...
  sec('O resumo do dev \u2014 a regra, os quatro campos e o texto do grupo');
  {
    /* A REGRA VIROU ARQUIVO quando o painel do dev pediu o mesmo resumo. Estas
       invariantes executam `resumo-dev.js`, e nao um recorte do gantt: e o
       arquivo que as duas telas usam. */
    const RD = require('./resumo-dev.js');
    global.PRAZO = require('./prazo.js');
    global.ETAPA = require('./etapa-demanda.js');
    global.DEVNOME = require('./dev-nome.js');
    ...
    const state = {
      temas: [{ id: 't1', nome: 'AXCred - Cobran\u00e7a' }],
      melhorias: [
        ...
        { id: '2', codigo: 'AX-145', titulo: 'Controle de cheques', dev: 'Dan Weine',
          tema_id: 't1', status_planejamento: 'em_andamento', entrega: dia(-2) },
        { id: '3', codigo: 'AX-317', titulo: 'Dois cliques', dev: 'Dan Weine',
          tema_id: 't1', status_planejamento: 'em_andamento', entrega: dia(5) },
        { id: '4', codigo: 'AX-401', titulo: 'Sem data', dev: 'Dan Weine',
          tema_id: 't1', status_planejamento: 'em_andamento' },
        { id: '5', codigo: 'AX-500', titulo: 'Pausada', dev: 'Dan Weine',
          tema_id: 't1', status_planejamento: 'em_andamento', entrega: dia(-30),
          pausado_em: dia(-25) },
        { id: '6', codigo: 'AX-316', titulo: 'Ajuste de nomes', dev: 'Dan Weine',
          tema_id: 't1', status_planejamento: 'validacao', entrega: dia(-10),
          entregue_em: dia(-12) + 'T14:00:00Z' },
        { id: '7', codigo: 'AX-600', titulo: 'De outra pessoa', dev: 'Gabriel',
          tema_id: 't1', status_planejamento: 'em_andamento', entrega: dia(-3) },
        { id: '8', codigo: 'AX-700', titulo: 'Concluida', dev: 'Dan Weine',
          tema_id: 't1', status_planejamento: 'concluido', entrega: dia(-40) },
        { id: '9', codigo: 'AX-800', titulo: 'Backlog', dev: 'Dan Weine',
          tema_id: 't1', status_planejamento: 'backlog' },
        { id: '10', codigo: 'AX-900', titulo: 'Oculta', dev: 'Dan Weine', oculto: true,
          tema_id: 't1', status_planejamento: 'em_andamento', entrega: dia(-3) },
    ...
    /* VALIDACAO NAO E ATRASO DO DEV \u2014 quem decide e o `prazo.js`. A AX-316
       ...
       pessoa por uma espera que nao e dela. */
    ok(!cods(r.atrasado).includes('AX-316'),
       'validacao com prazo vencido NAO entra em atrasado');
    /* PAUSADA tambem nao atrasa \u2014 o prazo esta suspenso, nao estourado. */
    ...
    ok(cods(r.andamento)[cods(r.andamento).length - 1] === 'AX-401',
    ...
    /* ─── OS QUATRO CAMPOS ────────────────────────────────────────────────
       ...
       encaminha UMA linha para outra conversa, o cabecalho do bloco fica para
       tras. */
    const x = r.atrasado[0];
    ok(x.codigo === 'AX-318', 'a linha traz o codigo');
    ok(!!x.tema, 'traz o sistema/modulo', x.tema);
    ok(!!x.temaCheio, 'e guarda o nome inteiro para o title da celula', x.temaCheio);
    ok(!!x.titulo, 'traz o titulo');
    ok(x.situacao && x.situacao.rot === 'Atrasado' && /7 dias/.test(x.situacao.det),
       'e traz a situacao, com rotulo e detalhe',
       x.situacao ? x.situacao.rot + ' / ' + x.situacao.det : '(sem situacao)');
    for (const y of [...r.andamento, ...r.validacao]) {
      ok(y.situacao && y.situacao.rot && y.situacao.det,
         'toda linha tem situacao preenchida (' + y.codigo + ')',
         y.situacao ? y.situacao.rot : '');
    }
    /* A MESMA `situacao` VAI PARA A TELA E PARA O TEXTO. Montada duas vezes,
       divergiria na primeira mudanca \u2014 e o recado passaria a dizer algo
       diferente do que a tela mostrou para quem o mandou. */
    const htm = RD.html(r), txt = RD.texto(r);
    ...
    ok(/- AX-318 \[AXCred/.test(txt),
       'a linha traz codigo e sistema entre colchetes',
       (txt.match(/- AX-318[^\n]*/) || [''])[0]);
    ok(/AX-318[^\n]*Anexo no historico[^\n]*Atrasado, 7 dias, venceu 09\/09\/2026/.test(txt),
       'e os quatro campos na mesma linha, nesta ordem');
    ...
    ok(/ATRASADO/.test(soAtraso) && !/EM ANDAMENTO/.test(soAtraso),
       'bloco sem nada nao aparece no texto');
    ok(/Nada em aberto/.test(RD.texto(RD.montar({ temas: [], melhorias: [] }, 'X', HOJE))),
  ...
  {
    /* AS DUAS TELAS USAM O MESMO ARQUIVO \u2014 e nenhuma tem copia da regra.
       Uma segunda copia teria a mesma vida curta das quatro da etapa e das
       quatro do prazo. */
    for (const [nome, txt] of [['gantt', GANTT], ['dev', DEV]]) {
      ok(txt.indexOf('resumo-dev.js') > 0, 'o ' + nome + ' carrega o resumo-dev.js');
      ok(/RESUMODEV\.abrir\(/.test(semComentario(txt)),
         'e o ' + nome + ' abre o resumo pelo modulo');
      ok(!/function gDevResumo\(/.test(txt),
         'e o ' + nome + ' NAO tem copia local da regra');
    }
    /* O gantt abre pelo NOME na faixa; o painel do dev, por um botao \u2014 la ha
       uma pessoa so, e nao ha nome para clicar. */
    ok(/class="dev-abrir"/.test(GANTT) && /<button type="button" class="dev-abrir"/.test(GANTT),
       'no gantt o nome do dev e um <button>, e o teclado chega nele');
    ok(/RESUMODEV\.abrir\(state, currentDev, \{ proprio: true \}\)/.test(DEV),
  ...
  sec('O grupo no gantt: a D0 por fora, as quebras dentro');
  {
    ...
    if (fnPack) {
       ...
       * este caso nao existiria. */
      {
        ...
        ok(t4.get('W') === 0,
           'quem cabe no vazio ao lado da D0 ocupa esse vazio, em vez de abrir ' +
           'faixa nova', 'W=' + t4.get('W'));
       ...
       * ela — e e o que o empacotador usa hoje. */
      {
        ...
        const f5 = new Map([
          ['D0', { sIdx: 14, eIdx: 17 }],           // a D0, no fim do mes
          ['Q1', { sIdx: 1, eIdx: 7 }],             // a quebra dela, no comeco
          ['M', { sIdx: 9, eIdx: 12 }],             // o MIOLO: entre a quebra e a D0
          ['M2', { sIdx: 9, eIdx: 12 }],            // outro igual, que nao cabe junto
          ['N', { sIdx: 18, eIdx: 20 }],            // depois da D0
          ['P', { sIdx: 15, eIdx: 16 }],            // debaixo da barra da D0
        ]);
        let tr5 = [];
        const t5 = pack(s5, f5, tr5, new Set());
        ok(t5.get('D0') === 0 && t5.get('Q1') === 1,
